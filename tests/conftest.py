...
def rupture_data_factory() -> Callable[..., pd.DataFrame]:
    ...
    def _create_rupture_data(
        num_sites: int = 1, custom_values: Optional[Dict[str, Any]] = None
    ) -> pd.DataFrame:
    ...
    try:
        ...
        for attr in required_attributes:
            if not hasattr(model_class, attr):
                return (
                    False,
                    f"Model {model_name} is missing required attribute: {attr}",
                )
        ...
        if not model_class.DEFINED_FOR_INTENSITY_MEASURE_TYPES:
            return (
                False,
                f"Model {model_name} doesn't define any intensity measure types",
            )
...
def openquake_test_wrapper(func):
    ...
    error messages without requiring a return value from the test.
    ...
    def test_my_gmm(mock_openquake_api, test_rupture_data):
        ...
        # Simply use assertions, no need to return anything
    ...
    def wrapper(*args, **kwargs):
        ...
        # Create a wrapper that will capture the return value
        ...
        def capturing_func(*args, **kwargs):
            ...
            # Don't return anything to avoid pytest warnings

        # Replace the original function with our capturing function temporarily
        wrapper.__wrapped__ = capturing_func

        try:
            # Call the capturing function (which doesn't return anything)
            capturing_func(*args, **kwargs)

            # Get the captured result for validation
            ...
            if isinstance(result, pd.DataFrame):
                ...
                # Check that there are no NaN values in the result
                assert not pd.isna(result).any().any(), "Result contains NaN values"

            # For other types of results, perform basic validations if applicable
            elif result is not None:
                if hasattr(result, "__len__"):
                    assert len(result) > 0, "Result is empty"
                if isinstance(result, (tuple, list)):
                    for item in result:
                        if isinstance(item, (float, np.number)):
                            assert not np.isnan(item), "Result contains NaN values"
                            assert not np.isinf(item), "Result contains infinite values"

            # Successful validation doesn't return anything
        except Exception as e:
...
class MockPlane:
    """Mock Plane class for testing fault plane calculations.

    This is a simplified version of source_modelling.sources.Plane
    for testing purposes. It provides just enough attributes and
    methods to test fault plane calculations without requiring
    ...
    def __init__(self, strike: float, dip: float):
        """Create a mock plane with specified strike and dip.
    ...
    def __repr__(self) -> str:
        """Return string representation of the mock plane."""
        return f"MockPlane(strike={self.strike}, dip={self.dip})"
