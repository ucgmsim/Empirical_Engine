...
def test_estimate_z2p5(test_z1p0, test_z1p5, expected_z2p5):
    assert round(classdef.estimate_z2p5(test_z1p0, test_z1p5), DIGITS) == round(expected_z2p5, DIGITS)
...
def test_interpolate_to_closest(test_t, test_t_high, test_t_low, test_y_high, test_y_low, expected):
    sa, sigma_sas = classdef.interpolate_to_closest(test_t, test_t_high, test_t_low, test_y_high, test_y_low)
    assert (round(sa, DIGITS), [round(sigma_sa, DIGITS) for sigma_sa in sigma_sas]) == (round(expected[0], DIGITS), [round(s, DIGITS) for s in expected[-1]])
