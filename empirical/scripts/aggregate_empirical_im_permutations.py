...
def load_args():
    ...
    parser.add_argument(
        "realisation", type=str, help="The realisation to generate empiricals for"
    )
    parser.add_argument(
        "simulation_root",
        type=absolute_path,
...
def agg_emp_perms(
    ...
    :param realisation: The realisation to generate the aggregated empirical files for
    :param simulation_root: The root of the simulation directory
    ...
    verification_dir = pathlib.Path(
        get_realisation_verification_dir(simulation_root, realisation)
    )
    ...
    for group in groups:
        identifier = get_agg_identifier(realisation, group)
        aggregation_logger.debug(
            f"The identifier {identifier} is being used for the IM group {group}"
        )
        aggregate_data(group, verification_dir, identifier, realisation, version)
        aggregation_logger.debug(
...
def main():
    ...
    add_general_file_handler(
        ...
        pathlib.Path(get_sim_dir(args.simulation_root, args.realisation))
        / "empirical_aggregation_log.txt",
    )
    main_aggregation_logger.debug(
        f"Loaded arguments, creating aggregated empiricals for input: {args}"
    )
    agg_emp_perms(
        args.realisation, args.simulation_root, args.version, main_aggregation_logger
    )
