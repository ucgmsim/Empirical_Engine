...
def calculate_ds(background_file, ll_file, vs30_file, ims, psa_periods, output_dir):
    ...
    rupture_list = [
        create_rupture_name(
            row.source_lat, row.source_lon, row.source_depth, mag, row.tect_type
        )
        for __, row in background_data.iterrows()
        for mag in get_mw_range(row.M_min, row.M_cutoff, row.n_mags)
    ]
    fault_list = [
        create_fault_name(row.source_lat, row.source_lon, row.source_depth)
        for __, row in background_data.iterrows()
    ]
    ...
    with pd.HDFStore(imdb_path) as im_store, pd.HDFStore(emp_ds_path) as distance_store:
        im_store["sites"] = site_df[["lat", "lon"]]
        distance_store["sites"] = pd.DataFrame(site_df.index)

        for index, station in site_df.iterrows():
            print(
                "Processing site {} / {}".format(
                    site_df.index.get_loc(index), n_stations
                )
            )
            site_im_df, distance_df = calculate_ds_site(
                ims,
                psa_periods,
                rupture_df,
                fault_df,
...
def calculate_ds_site(
    ims, psa_periods, rupture_df, fault_df, background_data, lat, lon, vs30
):
    im_df = pd.DataFrame()
    ...
    for index, row in background_data.iterrows():
        ...
        if rjb < MAX_RJB:
            ...
            for mag in get_mw_range(row.M_min, row.M_cutoff, row.n_mags):
                fault.Mw = mag
                rupture_name = create_rupture_name(
                    row.source_lat, row.source_lon, row.source_depth, mag, row.tect_type
                )
            ...
            fault_name = create_fault_name(
                row.source_lat, row.source_lon, row.source_depth
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--store_erf", action="store_true", help="writes the erf to a file"
    )
    parser.add_argument(
        "--skip_calculation",
        dest="run_calculation",
        action="store_false",
