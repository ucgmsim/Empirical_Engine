...
def CB_2014_nga(M, T, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, lamda, Fhw, Vs30, Z25, Zhyp, region):
    ...
    # initial A1100 value 999. A1100: median estimated value of PGA on rock with Vs30 = 1100m/s
    A1100 = 999

    # style of faulting
    Frv = int(30 < lamda < 150)
    Fnm = int(-150 < lamda < -30)
    ...
    if W is None:
        ...
        W = min(math.sqrt(10**((M-4.07)/0.98)),(Zbot - Ztori)/math.sin(math.pi/180*delta))    
        Zhyp = 9         
    ...
    if T == 1000:
        ...
        for ipT=1:length(period1)
            [Sa(ipT),sigma(ipT)]=CB_2014_nga_sub (M, ipT, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
            [PGA,~]=CB_2014_nga_sub (M, 22, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
    ...
    # compute Sa and sigma with user-defined period    
        ...
        for i=1:length(T)
            ...
            if ( isempty(find(abs(period-Ti) < 0.0001, 1))) % The user defined period requires interpolation
                ...
                [Sa_low, sigma_low] = CB_2014_nga_sub (M, ip_low, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
                [Sa_high, sigma_high] = CB_2014_nga_sub (M, ip_high, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
                [PGA,~]=CB_2014_nga_sub (M, 22, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
        
                x = [math.log(T_low) math.log(T_high)]
                Y_sa = [math.log(Sa_low) math.log(Sa_high)]
                Y_sigma = [sigma_low sigma_high]
                Sa[i] = math.exp(interp1(x, Y_sa, math.log(Ti)))
                sigma[i] = interp1(x, Y_sigma, math.log(Ti))
                
                if Sa[i]<PGA and period1[i]<0.25
                    Sa[i]= PGA
            else
                ip_T = find(abs((period- Ti)) < 0.0001)
                [Sa[i], sigma[i]] = CB_2014_nga_sub (M, ip_T, Rrup, Rjb, Rx, W, Ztor,Zbot,delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region,A1100)
                [PGA,~]=CB_2014_nga_sub (M, 22, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25, Zhyp,lamda, Frv, Fnm, region, A1100)
...
def CB_2014_nga_sub (M, ip, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, Fhw, Vs30, Z25in, Zhyp, lamda,Frv, Fnm, region, A1100):
    ...
    fmag = c0[ip] + c1[ip] * M + c2[ip] * max(0, (M - 4.5)) + c3[ip] * max(0, (M - 5.5)) + c4[ip] * max(0, (M-6.5))
    ...
    def hw_comp():
        if not Fhw:
            return 0
        R1 = W * math.cos(math.pi/180*delta) % W - downdip width

        if Rx < R1:
            f_hngRx = h1[ip]+h2[ip]*(Rx/R1)+h3[ip]*(Rx/R1)**2
        else:
            R2 = 62* M - 350
            f_hngRx = max(h4[ip]+h5[ip]*((Rx-R1)/(R2-R1))+h6[ip]*((Rx-R1)/(R2-R1))**2, 0)
    ...
    if Vs30 <= k1[ip]:
        if A1100 is None:
            A1100 = CB_2014_nga(M, 0, Rrup, Rjb, Rx, W, Ztor, Zbot, delta, lamda, Fhw, 1100, Z25A, Zhyp, region)
        f_siteG = c11[ip] * math.log(Vs30/k1[ip]) + k2[ip] * (math.log(A1100 + c * (Vs30/k1[ip])**n) - math.log(A1100 + c))
    elif Vs30 > k1[ip]:
        f_siteG = (c11[ip] + k2[ip] * n) * math.log(Vs30/k1[ip])

    if Vs30 <= 200:
        f_siteJ = (c12[ip]+k2[ip]*n)*(math.log(Vs30/k1[ip])-math.log(200/k1[ip]))*japan
    else:
    ...
    tau = math.sqrt(tau_lnyB**2 + alpha**2*tau_lnPGAB**2 + 2*alpha*rlnPGA_lnY[ip]*tau_lnyB*tau_lnPGAB)
    phi = math.sqrt(phi_lnyB**2 + flnAF[ip]**2 + alpha**2*phi_lnPGAB**2 + 2*alpha*rlnPGA_lnY[ip]*phi_lnyB*phi_lnPGAB)
